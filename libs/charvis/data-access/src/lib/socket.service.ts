...
export class SocketService {
  ...
  join(userName: string, roomName: string) {
    this.socket.emit(UserEvent.JoinRoom, {
      userName,
      roomName,
    } as UserJoinMessage);
  }
  ...
  usersUpdate(): Observable<UserOverview[]> {
    return this.socket
      .fromEvent<any>(ServerEvent.UsersUpdate)
  ...
  voteKick(userID: string) {
    this.socket.emit(UserEvent.VoteKick, {
      kickUserID: userID,
    } as UserVoteKickMessage);
  }
