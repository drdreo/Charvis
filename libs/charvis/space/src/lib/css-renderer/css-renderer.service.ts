...
export class CSSRendererService {
    ...
    private initGraphObjects(): void {
        for (const [index, data] of this.graphData.entries()) {
            const initObject = new CSS3DObject(
                this.graphToHTMLElement(data, this.graphTargets.init, index),
            );
    ...
    private htmlElement(table: any[], i: number) {
        ...
        element.addEventListener(
            "pointerdown",
            () => this.elementClickHandler(this.targets.base[i / 5], element),
            false,
        );
    ...
    private graphToHTMLElement(
        data: any,
        objectSource: Object3D[],
        index: number,
    ) {
        const element = document.createElement("div");
        element.className = "element";
        ...
        element.addEventListener(
            "pointerdown",
            () => this.elementClickHandler(objectSource[index], element),
            false,
        );
    ...
    private elementClickHandler(source: Object3D, element: HTMLElement) {
        ...
        this.transform(
            this.graphTargets.init,
            this.graphTargets.base,
            resetDuration,
        );
    ...
    private addSphereObject(
        sphereVector: Vector3,
        index: number,
        length: number,
    ) {
    ...
    private transformObjectPosition(
        object: Object3D,
    ...
    private transformObjectRotation(
        object: Object3D,
        targetObject: Object3D,
        duration: number,
    ) {
